...
namespace boingtheboeing.WebNowPlayingPlugin
{
    public class WNPReduxNative
    {
        ...
        public static void Start(int port)
        {
            ...
            optimisticUpdateTimer = new Timer(
        ...
        private static void PerformOptimisticUpdate(object state)
        {
            ...
            {
                if (
                    WNPRedux.MediaInfo._ID.StartsWith(idPrefix)
                    && WNPRedux.MediaInfo.State == MediaInfo.StateMode.PLAYING
                )
                {
                    WNPRedux.MediaInfo.PositionSeconds = (++lastPositionSeconds);
                    WNPRedux.MediaInfo.Position = WNPRedux.TimeInSecondsToString(
                        WNPRedux.MediaInfo.PositionSeconds
                    );

                    if (WNPRedux.MediaInfo.DurationSeconds > 0)
                    {
                        WNPRedux.MediaInfo.PositionPercent =
                            (
                                (double)WNPRedux.MediaInfo.PositionSeconds
                                / WNPRedux.MediaInfo.DurationSeconds
                            ) * 100.0;
                    }
                    else
                    {
        ...
        {
            if (!WNPRedux.MediaInfo._ID.StartsWith(idPrefix))
                return;
            try
            {
                ...
                switch (type)
                {
                    ...
                    case "TRY_SET_POSITION":
                        await session.TryChangePlaybackPositionAsync(
                            Convert.ToInt64(data.Split(':').First()) * 10_000_000
                        );
                        break;
                    case "TRY_SET_VOLUME":
                        break;
                    case "TRY_TOGGLE_REPEAT_MODE":
                        await session.TryChangeAutoRepeatModeAsync(
                        ...
                        break;
                    case "TRY_TOGGLE_SHUFFLE_ACTIVE":
                        await session.TryChangeShuffleActiveAsync(!mediaInfo.ShuffleActive);
                        break;
                    case "TRY_SET_RATING":
                        break;
                }
        ...
        static void SessionsChanged_Native(
            SessionManager manager,
            SessionsChangedEventArgs e = null
        )
        {
            foreach (Session session in sessions.Values)
            {
            ...
            foreach (Session session in _sessions)
            {
                ...
                {
                    if (IsAppIdBlacklisted(session.SourceAppUserModelId))
                        continue;
        ...
        static bool IsAppIdBlacklisted(string appId)
        {
            ...
            else if (id.Contains("308046B0AF4A39CB".ToLower()))
                return true; // firefox
            else if (id.Contains("6F193CCC56814779".ToLower()))
                return true; // firefox nightly
            else if (id.Contains("6F940AC27A98DD61".ToLower()))
                return true; // waterfox
            else if (id.Contains("A3665BA0C7D475A".ToLower()))
                return true; // pale moon

            return false;
        }

        static async void MediaPropertiesChanged_Native(
            Session session,
            MediaPropertiesChangedEventArgs e = null
        )
        {
        ...
        static void PlaybackInfoChanged_Native(
            Session session,
        ...
        )
        {
            ...
            string controls = "{";
            controls +=
                $"\"supports_play_pause\": {info.Controls.IsPlayPauseToggleEnabled.ToString().ToLower()},";
            controls +=
                $"\"supports_skip_previous\": {info.Controls.IsPreviousEnabled.ToString().ToLower()},";
            controls +=
                $"\"supports_skip_next\": {info.Controls.IsNextEnabled.ToString().ToLower()},";
            controls +=
                $"\"supports_set_position\": {info.Controls.IsPlaybackPositionEnabled.ToString().ToLower()},";
            controls += $"\"supports_set_volume\": false,";
            controls +=
                $"\"supports_toggle_repeat_mode\": {info.Controls.IsRepeatEnabled.ToString().ToLower()},";
            controls +=
                $"\"supports_toggle_shuffle_active\": {info.Controls.IsShuffleEnabled.ToString().ToLower()},";
            ...
            mediaInfo.State =
                info.PlaybackStatus == PlaybackStatus.Playing
                    ? MediaInfo.StateMode.PLAYING
                    : MediaInfo.StateMode.PAUSED;
            mediaInfo.ShuffleActive = info.IsShuffleActive ?? false;
            mediaInfo.RepeatMode =
                info.AutoRepeatMode == null || info.AutoRepeatMode.Value == AutoRepeatMode.None
                    ? MediaInfo.RepeatModeEnum.NONE
                    : info.AutoRepeatMode.Value == AutoRepeatMode.Track
                        ? MediaInfo.RepeatModeEnum.ONE
                        : MediaInfo.RepeatModeEnum.ALL;
            WNPRedux.UpdateMediaInfo();
        }

        static void TimelinePropertiesChanged_Native(
            Session session,
            TimelinePropertiesChangedEventArgs e = null
        )
        {
            ...
            if (lastPositionSeconds == Convert.ToInt32(info.Position.TotalSeconds))
                return;
            ...
            if (mediaInfo.DurationSeconds > 0)
            {
                mediaInfo.PositionPercent =
                    ((double)mediaInfo.PositionSeconds / mediaInfo.DurationSeconds) * 100.0;
            }
            else
            {
